...
(defun subject-distinct (subject &key (test (quote equal)))
  "Return a subject that emits only the first occurrence of each value (per TEST).
Standard test designators use hash lookup where equality is mutation-stable;
custom predicates and structural EQUAL/EQUALP values retain MEMBER semantics."
  (let* ((result (make-subject))
    ...
    (subject-subscribe
      ...
      (lambda (value)
        (let ((hashable (%distinct-hashable-value-p standard-test value)))
          (if hashable (multiple-value-bind (present-value present-p) (gethash value hashed-seen)
...
not."
  (let ((result (make-subject))
    ...
    (subject-subscribe
      ...
      (lambda (value)
        (when active
          (if (funcall predicate value) (subject-emit result value)
            (setf active nil)))))
    result))
