                 ...
                 :transitions (list
                               (cl-dataflow:make-transition
                                ...
                                :action (lambda (machine event context)
                                          ...
                                          (values "running"
                                                  '(:note "entered running"))))
                               (cl-dataflow:make-transition "running" "complete" "completed"))))
       (context (cl-dataflow:make-context :state (cl-dataflow:state-machine-state machine))))
  (multiple-value-bind (updated-machine transition-records updated-context)
      (cl-dataflow:run-state-machine-with-context machine '("start" "complete")
                                                  :context context)
    ...
    (format t "~&Last transition: ~S~%" (cl-dataflow:state-machine-last-transition machine))))
