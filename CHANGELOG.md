...
- New tests covering all ten modules (graph algorithms, export/serialization round trips, graph mutation/composition, state-machine analysis, combinator behavior including retry/fallback/memoization, stream laziness/purity/composition, stream generators/windowing/aggregates, pipeline/trace observability, state-machine execution, and effect ergonomics), bringing the suite from 189 to 334 with the coverage gate held at 100% branch / >=84% expression.
