...
- New tests covering all eight modules (graph algorithms, export/serialization round trips, graph mutation/composition, state-machine analysis, combinator behavior including retry/fallback/memoization, stream laziness/purity/composition, stream generators/windowing/aggregates, and pipeline/trace observability), bringing the suite from 189 to 322 with the coverage gate held at 100% branch / >=84% expression.
