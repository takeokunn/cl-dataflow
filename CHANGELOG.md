...
- New tests covering all nine modules (graph algorithms, export/serialization round trips, graph mutation/composition, state-machine analysis, combinator behavior including retry/fallback/memoization, stream laziness/purity/composition, stream generators/windowing/aggregates, pipeline/trace observability, and state-machine execution), bringing the suite from 189 to 327 with the coverage gate held at 100% branch / >=84% expression.
