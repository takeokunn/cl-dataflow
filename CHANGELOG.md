...
- New tests covering all seven modules (graph algorithms, export/serialization round trips, graph mutation/composition, state-machine analysis, combinator behavior including retry/fallback/memoization, stream laziness/purity/composition, and stream generators/windowing/aggregates), bringing the suite from 189 to 317 with the coverage gate held at 100% branch / >=84% expression.
