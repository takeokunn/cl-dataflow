...
(deftest
  ...
  (with-test-table
    ...
    (is
      (equal (cl-dataflow::%normalize-output-structure 7 '("value")) '(("value" . 7))))
    (is (equal (cl-dataflow::%hash-table-keys table) '("value" "other")))))
...
(deftest
  internal-error-copy-helpers-copy-nested-model-structures
  (let* ((node
        (make-node
          "source"
          :inputs
