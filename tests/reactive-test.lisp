...
(deftest
  subject-emit-empty-and-return-value
  (let ((subject (make-subject)))
    (is (eq (subject-emit subject :value) subject))))

(deftest
  subject-emit-self-unsubscribe-uses-entry-snapshot
  (let ((subject (make-subject))
        (seen '())
        self)
...
(deftest
  subject-emit-other-unsubscribe-uses-entry-snapshot
  (let ((subject (make-subject))
        (seen '())
        other)
...
(deftest
  subject-emit-subscribe-during-emit-uses-entry-snapshot
  (let ((subject (make-subject))
        (seen '())
        (subscribed nil))
    (subject-subscribe
...
(deftest
  subject-emit-reentrant-after-subscribe-sees-latest-registry
  (let ((subject (make-subject))
        (seen '()))
    (subject-subscribe
      subject
...
(deftest
  subject-emit-reentrant-after-unsubscribe-sees-latest-registry
  (let ((subject (make-subject))
        (seen '())
        second)
...
(deftest
  subject-emit-exception-skips-followers-and-retains-registry
  (let ((subject (make-subject))
        (seen '()))
    (subject-subscribe
      subject
...
(deftest
  subject-unsubscribe-removes-all-duplicate-subscriptions
  (let* ((subject (make-subject))
        (seen '())
        (duplicate
        (lambda (value)
