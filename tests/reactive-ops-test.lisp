...
(deftest
  subject-distinct-accepts-standard-function-designators
...
(deftest
  subject-distinct-preserves-custom-test-exploration
  (let* ((calls (quote ()))
          (source (make-subject))
          (collector
        (subject-collect
    ...
    (dolist (value (quote (1 2 1)))
      (subject-emit source value))
    (is (equal (funcall collector) (quote (1 2))))
    (is (equal (nreverse calls) (quote ((2 1) (1 2) (1 1)))))))

(deftest
  subject-distinct-retains-mutable-equal-semantics
  (let* ((value (copy-seq "a"))
          (source (make-subject))
          (collector (subject-collect (subject-distinct source :test (quote equal)))))
    (subject-emit source value)
    (setf (char value 0) #\b)
