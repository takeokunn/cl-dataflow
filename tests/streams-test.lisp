...
(deftest streams-construct-from-lists-and-varargs
...
(deftest stream-range-ascends-and-descends
...
(deftest stream-map-and-filter
  (is (equal (stream-collect (stream-map (lambda (x) (* x x)) (stream-of 1 2 3 4)))
             '(1 4 9 16)))
  (is (equal (stream-collect (stream-filter #'evenp (stream-of 1 2 3 4 5 6)))
             '(2 4 6))))

(deftest stream-scan-emits-running-accumulations
  (is (equal (stream-collect (stream-scan #'+ 0 (stream-of 1 2 3)))
             '(0 1 3 6)))
  (is (equal (stream-collect (stream-scan #'+ 10 (empty-stream)))
             '(10))))
...
(deftest stream-concat-and-zip
  (is (equal (stream-collect (stream-concat (stream-of 1 2) (empty-stream) (stream-of 3)))
             '(1 2 3)))
  (is (equal (stream-collect (stream-zip (stream-of 1 2 3) (stream-of :a :b)))
             '((1 . :a) (2 . :b)))))

(deftest stream-tap-observes-each-element
  (let ((seen '()))
    (is (equal (stream-collect
                (stream-tap (lambda (x) (push x seen)) (stream-of 1 2 3)))
               '(1 2 3)))
...
(deftest streams-compose-into-pipelines
  (is (equal (stream-collect
              (stream-take 3
                (stream-filter #'evenp
